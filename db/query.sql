...
LEFT JOIN departments ON roles.department_id = departments.id -- ...the employees table will join (from left) the departments table with/ON the column role_id in the employees table which will then become the id column in the departments table..then..
...
SELECT roles.title, roles.salary, departments.name -- TO VIEW ROLE DATA. First, selecting title and salary in the roles table, then the name in the departments table...then..
FROM roles -- ..from the roles table..
JOIN departments ON roles.department_id = departments.id -- ..the roles table will join departments with/ON the departments_id in the roles table which will then be the id column in the departments table
